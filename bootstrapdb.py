...
def pay_salaries(session: Session):
    ...
    for employee in employees:
        employee.pay(amount=500)
    session.commit()


def create_departments(session: Session):
    departments = [
        "HR", "HQ", "Sales", "IT", "Marketing", "Supply", "Law", "Reclamation", "Support", "PR"
    ]
    for department in departments:
        session.add(
            Department(name=department, address="Wiejska 1, Warszawa")
        )
        try:
            session.commit()
        except IntegrityError:
...
def shuffle_employees(session: Session):
    ...
    for department in departments:
        department.employees.extend(choices(employees, k=20))
        try:
            session.commit()
        except IntegrityError:
            session.rollback()
